...
Selected mints are audited concurrently, but every RPC call shares one global
pacer. This overlaps network latency and sweep-gap waits without increasing the
declared RPC start rate.
"""
...
async def main_async(a):
    ...
    sem=asyncio.Semaphore(max(1,a.audit_concurrency))
    connector=aiohttp.TCPConnector(limit=max(16,a.audit_concurrency*2))
    async with aiohttp.ClientSession(connector=connector,headers={'User-Agent':'microcap-runner-ht-audit/0.3'}) as s:
        async def work(r):
    ...
    print(json.dumps({'audit_version':AUDIT_VERSION,'audit_concurrency':a.audit_concurrency,'rows':len(rows),'selected':len(selected),
...
def main():
    ...
    p.add_argument('--min-fraction',type=float,default=.98);p.add_argument('--audit-concurrency',type=int,default=8)
