...
Only rows selected before outcomes are eligible:
...
async def signatures_window(session,pacer,address,start_s,end_s,max_pages):
    ...
    while pages<max_pages:
        ...
        rows,e=await rpc(session,pacer,'getSignaturesForAddress',[address,opts])
        pages+=1
        ...
        if not before:break
    # Dedup signatures in deterministic time/order.
...
def eligible(row):
    if row.get('data_status')!='VALID':return False
...
def scan_rows(ledger:Path,horizon_s:int,now_ms:int):
    found=[]
    for p in sorted((ledger/'prospective'/'scans').glob('*/scored_rows.jsonl')) if (ledger/'prospective'/'scans').exists() else []:
...
async def track_one(session,pacer,row,horizon_s,max_pages,max_txs,entry_network_lamports,exit_network_lamports):
    ...
    q=row.get('paper_curve_quote') or {};tokens=int(q.get('tokens_out_raw') or 0);gross=int(q.get('gross_quote_in_raw') or 0)
    entry_avg=q.get('average_price_sol')
    ...
    entry_total=(gross+entry_network_lamports)/LAMPORTS
    # Start before action time so migration discovery cannot miss a transition at the boundary.
    start_s=max(0,scored_ms/1000-120)
    ...
    # Dedup swap events because migration/mint and pool histories can overlap.
    sd={}
    ...
    ppoints=pump_exit_points(pump_events,mint=mint,tokens_owned_raw=tokens,entry_total_outlay_sol=entry_total,
                             entry_average_price_sol=float(entry_avg),after_ms=scored_ms,exit_network_lamports=exit_network_lamports)
    spoints=pumpswap_exit_points(swap_events,pool=pool,tokens_owned_raw=tokens,entry_total_outlay_sol=entry_total,
                                 entry_average_price_sol=float(entry_avg),after_ms=scored_ms,exit_network_lamports=exit_network_lamports) if pool else []
    ...
    if pool is not None:
        complete=complete and bool(phist and not phist['errors'] and phist['reached_start_boundary'] and not phist['page_cap_hit'] and not ptxerr and not ptrunc)
    summary=summarize_executable_path(points)
    return {
        'status':'COMPLETE' if complete else 'OUTCOME_INCOMPLETE','mint':mint,'name':row.get('name'),'symbol':row.get('symbol'),
        'horizon_s':horizon_s,'scored_ms':scored_ms,'cutoff_ms':int(end_s*1000),'decision':row.get('decision'),
        'prospective_controls':row.get('prospective_controls'),'entry':{
            'quote_source':'action_time_last_observed_pump_state','gross_quote_in_raw':gross,'tokens_owned_raw':tokens,
            'average_price_sol':entry_avg,'entry_network_lamports':entry_network_lamports,'entry_total_outlay_sol':entry_total,
            'account_rent_included':False,
        },
        ...
        'outcome':summary if complete else None,
        'diagnostic_outcome_even_if_incomplete':summary,
...
async def main_async(a):
    ledger=Path(a.ledger_root);now_ms=int(time.time()*1000);todo=scan_rows(ledger,a.horizon_s,now_ms)
    pacer=Pacer(a.rpc_interval);results=[]
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30),headers={'User-Agent':'microcap-runner-outcomes/0.1'}) as session:
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--ledger-root',required=True);p.add_argument('--horizon-s',type=int,required=True)
    p.add_argument('--max-rows',type=int,default=12);p.add_argument('--max-pages',type=int,default=8);p.add_argument('--max-txs',type=int,default=2500)
    p.add_argument('--rpc-interval',type=float,default=.28);p.add_argument('--entry-network-lamports',type=int,default=5000);p.add_argument('--exit-network-lamports',type=int,default=5000)
